...
$createLogs = $createLogs ?? 0;
$updateLogs = $updateLogs ?? 0;
$deleteLogs = $deleteLogs ?? 0;
...
function actionBadgeClass(string $action): string {
    return match (strtoupper($action)) {
        ...
        'UPDATE', 'ARCHIVE', 'RESTORE' => 'bg-amber-50 text-amber-700 ring-1 ring-amber-200',
        ...
        'IMPORT' => 'bg-pink-50 text-pink-700',
...
<div class="space-y-6">
        ...
        <div class="relative px-6 py-8 sm:px-8">
            <div class="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    if ($filterAction !== '') $exportQuery['action'] = $filterAction;
                    if ($filterEntity !== '') $exportQuery['entity'] = $filterEntity;
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        ...
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($createLogs) ?></p>
                    </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        ...
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($updateLogs) ?></p>
                    </div>
                ...
                <div class="mt-4 flex items-center gap-2 text-xs">
                    <span class="rounded-full bg-amber-50 px-2 py-1 font-medium text-amber-700">UPDATE actions</span>
                </div>
            </div>
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        ...
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($deleteLogs) ?></p>
                    </div>
                ...
                <div class="mt-4 flex items-center gap-2 text-xs">
                    <span class="rounded-full bg-red-50 px-2 py-1 font-medium text-red-700">DELETE actions</span>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="rounded-2xl border border-gray-200 bg-white">
        <div class="flex flex-col gap-4 border-b border-gray-100 px-6 py-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <h2 class="font-semibold text-gray-900">Audit Trail</h2>
                <p class="mt-1 text-xs text-gray-500">
                    <?= $totalRows ?> record<?= $totalRows !== 1 ? 's' : '' ?> found
                </p>
            </div>
            <form method="GET" action="<?= BASE_URL ?>/master/audit-logs" class="flex flex-col gap-3 lg:items-end">
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
                    ...
                    <select name="action" class="rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm text-gray-800 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20">
                        ...
                        <option value="ARCHIVE" <?= $filterAction === 'ARCHIVE' ? 'selected' : '' ?>>ARCHIVE</option>
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                    ...
                    <div class="flex items-center gap-2 pt-3 sm:pt-5">
                        ...
                        <?php if ($search !== '' || $filterAction !== '' || $filterEntity !== '' || $filterDateFrom !== '' || $filterDateTo !== ''): ?>
                            <a href="<?= BASE_URL ?>/master/audit-logs"
                    ...
                    <?php if (empty($logs)): ?>
                        <tr>
                            <td colspan="7" class="px-6 py-16 text-center">
                                ...
                                <p class="mt-4 text-sm font-medium text-gray-700">No audit logs found.</p>
                                <p class="mt-1 text-xs text-gray-500">User actions will appear here as they are performed.</p>
                            </td>
                        </tr>
                    <?php else: ?>
                        ...
                        <tr class="hover:bg-gray-50/50 transition">
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center rounded-md px-2 py-0.5 text-[11px] font-bold tracking-wide <?= actionBadgeClass($l['action']) ?>">
                                    <?= htmlspecialchars($l['action']) ?>
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="max-w-md">
                                    <div class="font-medium text-gray-900 line-clamp-2"><?= htmlspecialchars($l['description'] ?? '—') ?></div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <?php if ($l['entity_type']): ?>
                                    <div class="inline-flex items-center rounded-md bg-indigo-50 px-2 py-0.5 text-[11px] font-semibold text-indigo-700">
                                        <?= htmlspecialchars($l['entity_type']) ?>
                                    </div>
                                    <?php if ($l['entity_id']): ?>
                                        <div class="mt-0.5 text-[11px] text-gray-400 font-mono">#<?= htmlspecialchars($l['entity_id']) ?></div>
                                    <?php endif; ?>
                                <?php else: ?>
                            ...
                            <td class="px-6 py-4">
                                <?php
                                $hasOld = $l['old_values'] && $l['old_values'] !== 'null' && $l['old_values'] !== '[]' && $l['old_values'] !== '{}';
                                $hasNew = $l['new_values'] && $l['new_values'] !== 'null' && $l['new_values'] !== '[]' && $l['new_values'] !== '{}';
                                ?>
                                <div class="flex flex-wrap gap-1">
                                    <?php if ($hasOld): ?>
                                        <span class="inline-flex items-center rounded-md bg-rose-50 text-rose-700 ring-1 ring-rose-200 px-1.5 py-0.5 text-[10px] font-semibold">OLD</span>
                                    <?php endif; ?>
                                    <?php if ($hasNew): ?>
                                        <span class="inline-flex items-center rounded-md bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200 px-1.5 py-0.5 text-[10px] font-semibold">NEW</span>
                                    <?php endif; ?>
                                    <?php if (!$hasOld && !$hasNew): ?>
                                        <span class="text-[11px] text-gray-400 italic">No diff</span>
                                    <?php endif; ?>
                                </div>
                            </td>
                            <td class="px-6 py-4">
...
<div id="logDetailModal" class="fixed inset-0 z-[100] hidden items-center justify-center" role="dialog" aria-modal="true">
    ...
    <div id="logDetailModalPanel" class="relative z-10 w-full max-w-3xl mx-4 bg-white rounded-2xl shadow-2xl border border-gray-100 transform scale-95 opacity-0 transition-all duration-200 max-h-[90vh] overflow-y-auto">
    ...
    <div id="purgeModalPanel" class="relative z-10 w-full max-w-md mx-4 bg-white rounded-2xl shadow-2xl border border-gray-100 transform scale-95 opacity-0 transition-all duration-200">
        <div class="p-6">
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                    <h3 class="text-lg font-semibold text-gray-900">Purge Old Audit Logs</h3>
                    <p class="mt-1 text-sm text-gray-600">This permanently deletes audit records older than the specified days. This action cannot be undone.</p>
                </div>
            </div>
            <div class="mt-5">
...
function actionBadgeClass(action) {
    const map = {
        ...
        'ARCHIVE': 'bg-amber-50 text-amber-700 ring-1 ring-amber-200',
        'RESTORE': 'bg-amber-50 text-amber-700 ring-1 ring-amber-200',
        'DELETE': 'bg-red-50 text-red-700 ring-1 ring-red-200',
        'LOGIN': 'bg-blue-50 text-blue-700',
        'LOGOUT': 'bg-gray-100 text-gray-700',
        'VIEW': 'bg-indigo-50 text-indigo-700',
...
function diffToHtml(oldVal, newVal) {
    const changed = [];
    const keys = new Set();
    if (isPlainObject(oldVal)) Object.keys(oldVal).forEach(k => keys.add(k));
    if (isPlainObject(newVal)) Object.keys(newVal).forEach(k => keys.add(k));
    keys.forEach(k => {
        const o = isPlainObject(oldVal) ? oldVal[k] : undefined;
        const n = isPlainObject(newVal) ? newVal[k] : undefined;
        const oStr = o === undefined ? '(not set)' : (typeof o === 'object' ? JSON.stringify(o) : String(o));
        const nStr = n === undefined ? '(not set)' : (typeof n === 'object' ? JSON.stringify(n) : String(n));
        if (oStr !== nStr) {
            changed.push(`
                <div class="space-y-1.5 py-2 border-b border-gray-100 last:border-0">
                    <div class="text-[11px] font-semibold uppercase tracking-wider text-gray-400">${k}</div>
                    <div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
                        <div class="rounded-lg bg-rose-50 border border-rose-200 p-2.5">
                            <div class="text-[10px] font-bold uppercase tracking-wider text-rose-500 mb-0.5">Old</div>
                            <div class="text-xs text-rose-800 font-mono break-all whitespace-pre-wrap">${String(oStr).replace(/</g, '&lt;')}</div>
                        </div>
                        <div class="rounded-lg bg-emerald-50 border border-emerald-200 p-2.5">
                            <div class="text-[10px] font-bold uppercase tracking-wider text-emerald-600 mb-0.5">New</div>
                            <div class="text-xs text-emerald-800 font-mono break-all whitespace-pre-wrap">${String(nStr).replace(/</g, '&lt;')}</div>
                        </div>
                    </div>
                </div>`);
        }
    });
    return changed.length ? changed.join('') : '<p class="text-xs text-gray-400 italic">No field-level changes detected.</p>';
}
...
async function viewLog(id) {
    ...
    openLogDetailModal();
    try {
        const res = await fetch(`<?= BASE_URL ?>/master/audit-logs/show?id=${id}`);
        ...
        document.getElementById('logDetailSubtitle').textContent = `Audit #${l.id} — ${l.created_at}`;

        const oldHtml = l.old_values && Object.keys(l.old_values).length
            ? `<pre class="mt-2 rounded-lg bg-gray-900 p-4 text-[11px] leading-relaxed text-gray-100 overflow-x-auto">${JSON.stringify(l.old_values, null, 2)}</pre>`
            : '<span class="text-xs text-gray-400 italic">No old values</span>';

        const newHtml = l.new_values && Object.keys(l.new_values).length
            ? `<pre class="mt-2 rounded-lg bg-gray-900 p-4 text-[11px] leading-relaxed text-gray-100 overflow-x-auto">${JSON.stringify(l.new_values, null, 2)}</pre>`
            : '<span class="text-xs text-gray-400 italic">No new values</span>';

        const hasDiff = (l.old_values && Object.keys(l.old_values).length) || (l.new_values && Object.keys(l.new_values).length);
        const diffHtml = hasDiff ? diffToHtml(l.old_values, l.new_values) : '<p class="text-xs text-gray-400 italic">No change data available.</p>';

        document.getElementById('logDetailContent').innerHTML = `
            <div class="space-y-5">
                <div class="flex flex-wrap items-center gap-2">
                    <span class="inline-flex items-center rounded-md px-2.5 py-1 text-[11px] font-bold tracking-wide ${actionBadgeClass(l.action)}">${l.action}</span>
                    ${l.entity_type ? `<span class="inline-flex items-center rounded-md bg-indigo-50 text-indigo-700 px-2 py-0.5 text-[11px] font-semibold">${l.entity_type}</span>` : ''}
                    ${l.entity_id ? `<span class="text-[11px] font-mono text-gray-500">#${l.entity_id}</span>` : ''}
                </div>

                ${l.description ? `
                <div class="rounded-xl border border-gray-200 bg-gray-50 p-4">
                    <p class="mb-1 text-[10px] font-semibold uppercase tracking-wider text-gray-400">Description</p>
                    <p class="text-sm text-gray-800 whitespace-pre-wrap">${l.description.replace(/</g, '&lt;')}</p>
                </div>` : ''}

                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <div class="rounded-xl border border-gray-200 p-4">
                        <p class="mb-1 text-[10px] font-semibold uppercase tracking-wider text-gray-400">Performed By</p>
                    ...
                    <div class="rounded-xl border border-gray-200 p-4">
                        ...
                        <p class="text-xs text-gray-600 truncate" title="${l.user_agent || ''}">${l.user_agent || '—'}</p>
                    </div>
                </div>

                ${hasDiff ? `
                <div class="rounded-xl border border-gray-200 p-4">
                    <p class="mb-2.5 text-[10px] font-semibold uppercase tracking-wider text-gray-400">Field-by-Field Diff</p>
                    <div class="space-y-1">${diffHtml}</div>
                </div>` : ''}

                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <div class="rounded-xl border border-gray-200 p-4">
                        <p class="mb-1.5 text-[10px] font-semibold uppercase tracking-wider text-gray-400">Old Values (JSON)</p>
                        ${oldHtml}
                    </div>
                    <div class="rounded-xl border border-gray-200 p-4">
                        <p class="mb-1.5 text-[10px] font-semibold uppercase tracking-wider text-gray-400">New Values (JSON)</p>
                        ${newHtml}
                    </div>
                </div>
            </div>
...
async function executePurge() {
    ...
    try {
        ...
        fd.append('days', days);
        const res = await fetch('<?= BASE_URL ?>/master/audit-logs/clear-old', { method: 'POST', body: fd });
        const data = await res.json();
