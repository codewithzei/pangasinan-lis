...
function openDocumentStatusModal() {
    ...
    title.textContent = 'Create New Document Status';
...
async function editDocumentStatus(id) {
    ...
    title.textContent = 'Edit Document Status';
...
function deleteDocumentStatus(id, name) {
    openConfirmModal('Delete Document Status', `Are you sure you want to delete "${name}"?`, async () => {
        ...
        const result = await response.json();
        if (result.success) {
            showToast(result.message || 'Document status deleted successfully.', 'success');
            setTimeout(() => location.reload(), 500);
        } else {
            showToast(result.message || 'Failed to delete document status.', 'error');
        }
    });
}
...
function toggleStatus(id, name) {
    openConfirmModal('Update Status', `Change the status of "${name}"?`, async () => {
        ...
        const result = await response.json();
        if (result.success) {
            showToast(result.message || 'Document status updated successfully.', 'success');
            setTimeout(() => location.reload(), 500);
        } else {
            showToast(result.message || 'Unable to update document status.', 'error');
        }
    });
}
