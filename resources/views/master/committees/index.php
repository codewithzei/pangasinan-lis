...
$error = $error ?? null;
$pageTitle = $pageTitle ?? 'Committees Management';
$pageSubtitle = $pageSubtitle ?? '';
$totalCommittees = $totalCommittees ?? 0;
$activeCommittees = $activeCommittees ?? 0;
$inactiveCommittees = $inactiveCommittees ?? 0;
...
<div class="space-y-6">
        ...
        <div class="relative px-6 py-8 sm:px-8">
            <div class="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    <button type="button" onclick="openCommitteeModal()"
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        <p class="text-sm text-gray-500">Total Committees</p>
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($totalCommittees) ?></p>
                    </div>
                ...
                <div class="mt-4 flex items-center gap-2 text-xs">
                    <span class="rounded-full bg-blue-50 px-2 py-1 font-medium text-blue-600">All committees</span>
                </div>
            </div>
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        <p class="text-sm text-gray-500">Active Committees</p>
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($activeCommittees) ?></p>
                    </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        <p class="text-sm text-gray-500">Inactive Committees</p>
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($inactiveCommittees) ?></p>
                    </div>
    ...
    <section class="rounded-2xl border border-gray-200 bg-white">
        <div class="flex flex-col gap-4 border-b border-gray-100 px-6 py-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <h2 class="font-semibold text-gray-900">Committees List</h2>
                <p class="mt-1 text-xs text-gray-500">
                    <?= $totalRows ?> record<?= $totalRows !== 1 ? 's' : '' ?> found
                </p>
            </div>
            <form method="GET" action="<?= BASE_URL ?>/master/committees" class="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div class="relative">
                    ...
                    <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Search committee name..."
                ...
                <?php if ($search !== '' || $filterStatus !== ''): ?>
                    <a href="<?= BASE_URL ?>/master/committees"
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm">
                <thead class="bg-gray-50 text-xs uppercase text-gray-500">
                    <tr>
                        <th class="px-6 py-3 font-medium">Committee</th>
                    ...
                    <?php if (empty($committees)): ?>
                        <tr>
                            <td colspan="4" class="px-6 py-16 text-center">
                                ...
                                <p class="mt-4 text-sm font-medium text-gray-700">No committees found.</p>
                                <p class="mt-1 text-xs text-gray-500">Create your first committee to populate dropdowns.</p>
                                <button type="button" onclick="openCommitteeModal()"
                        ...
                        <tr class="hover:bg-gray-50/50 transition">
                            <td class="px-6 py-4">
                                <div>
                                    ...
                                    <?php if (!empty($c['description'])): ?>
                                        <div class="mt-0.5 text-xs text-gray-500 line-clamp-1 max-w-lg"><?= htmlspecialchars($c['description']) ?></div>
                                    <?php endif; ?>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    <button type="button" onclick="editCommittee(<?= $c['id'] ?>)"
                                    ...
                                    <button type="button" onclick="toggleStatus(<?= $c['id'] ?>, '<?= htmlspecialchars($c['name']) ?>')"
                                    ...
                                    <button type="button" onclick="deleteCommittee(<?= $c['id'] ?>, '<?= htmlspecialchars($c['name']) ?>')"
    ...
    <div id="committeeModalPanel" class="relative z-10 w-full max-w-xl mx-4 bg-white rounded-2xl shadow-2xl border border-gray-100 transform scale-95 opacity-0 transition-all duration-200 max-h-[90vh] overflow-y-auto">
        <form id="committeeForm" method="POST" action="<?= BASE_URL ?>/master/committees/store">
            ...
            <div class="flex items-center justify-between border-b border-gray-100 px-6 py-5">
                <div>
                    <h3 id="committeeModalTitle" class="text-lg font-semibold text-gray-900">Create New Committee</h3>
                    <p class="mt-0.5 text-xs text-gray-500">Fill in the committee details below.</p>
                </div>
                <button type="button" onclick="closeCommitteeModal()" class="rounded-lg p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="p-6 space-y-5">
                <div>
                    ...
                    <input type="text" name="name" id="committeeName" value="<?= old('name') ?>"
                        ...
                        placeholder="e.g. Committee on Rules" required maxlength="50">
                    <p class="mt-1 text-[11px] text-gray-500">Maximum 50 characters.</p>
                </div>
                <div>
                    <label class="mb-1.5 block text-xs font-semibold text-gray-700">Description / Notes</label>
                    <textarea name="description" id="committeeDescription" rows="3"
                        ...
                        placeholder="Optional description about this committee..."><?= old('description') ?></textarea>
                </div>
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="number" name="sort_order" id="sortOrder" min="0" value="<?= old('sort_order', '0') ?>"
                            class="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm text-gray-800 placeholder-gray-400 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
                            placeholder="0">
                        <p class="mt-1 text-[11px] text-gray-500">Committees with lower values appear first in dropdowns.</p>
                    </div>
                    <div class="flex items-end">
                        <div class="flex items-center gap-3 rounded-xl border border-gray-200 bg-gray-50 p-4 w-full">
                            ...
                            <div>
                                ...
                                <p class="text-[11px] text-gray-500 mt-0.5">Active committees appear in selection dropdowns.</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex items-center justify-end gap-3 border-t border-gray-100 px-6 py-4 bg-gray-50 rounded-b-2xl">
                ...
                <button type="submit" id="committeeSubmitBtn"
...
function validateCommitteeForm() {
    ...
    const name = document.getElementById('committeeName');
    if (name.value.trim() === '') {
        setFieldError('committeeName', 'Committee name is required.');
        valid = false;
    } else if (name.value.length > 50) {
        setFieldError('committeeName', 'Committee name must not exceed 50 characters.');
        valid = false;
    } else clearFieldError('committeeName');
...
function attachCommitteeValidation() {
    const fields = ['committeeName', 'sortOrder'];
    fields.forEach(id => {
        const el = document.getElementById(id);
        if (!el) return;
        el.addEventListener('input', validateCommitteeForm);
        el.addEventListener('blur', validateCommitteeForm);
        el.addEventListener('change', validateCommitteeForm);
    });
...
function resetCommitteeForm() {
    ...
    form.action = '<?= BASE_URL ?>/master/committees/store';
    document.getElementById('committeeModalTitle').textContent = 'Create New Committee';
    document.getElementById('committeeSubmitBtn').innerHTML = `
        <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
        </svg>
        Save
    `;
    ['committeeName', 'sortOrder'].forEach(clearFieldError);
    attachCommitteeValidation();
}
function openCommitteeModal() {
    ...
    setTimeout(() => document.getElementById('committeeName').focus(), 220);
}
...
async function editCommittee(id) {
    try {
        const res = await fetch(`<?= BASE_URL ?>/master/committees/edit?id=${id}`);
        const data = await res.json();
        if (!data.success) {
            showToast(data.message || 'Committee not found.', 'error');
            return;
        }
        const c = data.data;
        document.getElementById('committeeId').value = c.id;
        document.getElementById('committeeName').value = c.name;
        document.getElementById('committeeDescription').value = c.description || '';
        document.getElementById('sortOrder').value = c.sort_order ?? 0;
        document.getElementById('isActive').checked = !!c.is_active;
        document.getElementById('committeeForm').action = '<?= BASE_URL ?>/master/committees/update';
        ...
        showToast('Failed to load committee data.', 'error');
    }
}
    ...
    openConfirmModal({
        ...
        onConfirm: async () => {
            try {
                ...
                fd.append('id', id);
                const res = await fetch('<?= BASE_URL ?>/master/committees/destroy', { method: 'POST', body: fd });
                const data = await res.json();
...
async function toggleStatus(id, name) {
    openConfirmModal({
        title: 'Toggle Status',
        message: `This will toggle the active status of <strong class="text-gray-800">${name}</strong>.<br><span class="text-xs text-gray-500">Inactive committees are hidden from selection dropdowns.</span>`,
        confirmText: 'Proceed',
        type: 'info',
        onConfirm: async () => {
            try {
                const fd = new FormData();
                fd.append('id', id);
                const res = await fetch('<?= BASE_URL ?>/master/committees/toggle-status', { method: 'POST', body: fd });
                const data = await res.json();
                showToast(data.message, data.success ? 'success' : 'error');
