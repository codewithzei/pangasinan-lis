...
$error = $error ?? null;
$pageTitle = $pageTitle ?? 'Document Types Management';
$pageSubtitle = $pageSubtitle ?? '';
$totalDocumentTypes = $totalDocumentTypes ?? 0;
$activeDocumentTypes = $activeDocumentTypes ?? 0;
$inactiveDocumentTypes = $inactiveDocumentTypes ?? 0;
...
<div class="space-y-6">
    ...
    <section class="overflow-hidden rounded-2xl bg-linear-to-br from-primary via-primary to-blue-700 shadow-md">
        <div class="relative px-6 py-8 sm:px-8">
            <div class="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                <div class="max-w-3xl">
                    <p class="text-sm font-medium text-blue-100">DOCUMENT TYPE MASTER DATA</p>
                ...
                <div class="flex flex-wrap items-center gap-3">
                    <button type="button" onclick="openDocumentTypeModal()"
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        <p class="text-sm text-gray-500">Total Document Types</p>
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($totalDocumentTypes) ?></p>
                    </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        <p class="text-sm text-gray-500">Active Document Types</p>
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($activeDocumentTypes) ?></p>
                    </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        <p class="text-sm text-gray-500">Inactive Document Types</p>
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($inactiveDocumentTypes) ?></p>
                    </div>
    ...
    <section class="rounded-2xl border border-gray-200 bg-white">
        <div class="flex flex-col gap-4 border-b border-gray-100 px-6 py-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <h2 class="font-semibold text-gray-900">Document Types List</h2>
                <p class="mt-1 text-xs text-gray-500">
                    <?= $totalRows ?> record<?= $totalRows !== 1 ? 's' : '' ?> found
                </p>
            </div>
            <form method="GET" action="<?= BASE_URL ?>/master/document-types" class="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div class="relative">
                    <svg class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                    </svg>
                    <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Search document type..."
                ...
                <?php if ($search !== '' || $filterStatus !== ''): ?>
                    <a href="<?= BASE_URL ?>/master/document-types"
                ...
                <tbody class="divide-y divide-gray-100">
                    <?php if (empty($documentTypes)): ?>
                        <tr>
                            <td colspan="5" class="px-6 py-16 text-center">
                                ...
                                <p class="mt-4 text-sm font-medium text-gray-700">No document types found.</p>
                    ...
                    <?php else: ?>
                        <?php foreach ($documentTypes as $documentType): ?>
                            <?php $badgeColor = !empty($documentType['badge_color']) ? $documentType['badge_color'] : '#2563EB'; ?>
                            <tr class="hover:bg-gray-50/50 transition">
                                <td class="px-6 py-4">
                                    <div>
                                        <div class="font-medium text-gray-900"><?= htmlspecialchars($documentType['name']) ?></div>
                                        <?php if (!empty($documentType['description'])): ?>
                                            <div class="mt-0.5 text-xs text-gray-500 line-clamp-1 max-w-lg"><?= htmlspecialchars($documentType['description']) ?></div>
                                        <?php endif; ?>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        ...
                                        <button type="button" onclick="toggleStatus(<?= $documentType['id'] ?>, '<?= htmlspecialchars($documentType['name']) ?>')"
                                            ...
                                            title="<?= $documentType['is_active'] ? 'Set Inactive' : 'Set Active' ?>">
                                            <?php if ($documentType['is_active']): ?>
                                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <button type="button" onclick="deleteDocumentType(<?= $documentType['id'] ?>, '<?= htmlspecialchars($documentType['name']) ?>')"
...
<div id="documentTypeModal" class="fixed inset-0 z-100 hidden items-center justify-center" role="dialog" aria-modal="true">
    ...
    <div id="documentTypeModalPanel" class="relative z-10 w-full max-w-xl mx-4 bg-white rounded-2xl shadow-2xl border border-gray-100 transform scale-95 opacity-0 transition-all duration-200 max-h-[90vh] overflow-y-auto">
        <form id="documentTypeForm" method="POST" action="<?= BASE_URL ?>/master/document-types/store">
            <input type="hidden" name="id" id="documentTypeId" value="">
            <div class="flex items-center justify-between border-b border-gray-100 px-6 py-5">
                <div>
                    <h3 id="documentTypeModalTitle" class="text-lg font-semibold text-gray-900">Create New Document Type</h3>
                    <p class="mt-0.5 text-xs text-gray-500">Fill in the document type details below.</p>
                </div>
                <button type="button" onclick="closeDocumentTypeModal()" class="rounded-lg p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="p-6 space-y-5">
                <div>
                    ...
                    <input type="text" name="name" id="documentTypeName" value="<?= old('name') ?>"
                        ...
                        placeholder="e.g. Provincial Ordinance" required>
                </div>

                <div>
                    <label class="mb-1.5 block text-xs font-semibold text-gray-700">Description / Notes</label>
                    <textarea name="description" id="documentTypeDescription" rows="3"
                        ...
                        placeholder="Optional description for this document category..."><?= old('description') ?></textarea>
                </div>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            <input type="color" name="badge_color" id="documentTypeBadgeColor" value="<?= old('badge_color', '#2563EB') ?>" class="h-11 w-16 cursor-pointer rounded-lg border border-gray-200 bg-white p-1">
                            <input type="text" id="documentTypeBadgeHex" value="<?= old('badge_color', '#2563EB') ?>" maxlength="7"
                        ...
                        <p class="mt-1 text-[11px] text-gray-500">This color is used for the badge tag in document listings.</p>
                    </div>
                    <div>
                        <label class="mb-1.5 block text-xs font-semibold text-gray-700">
                            Sort Order
                        </label>
                        <input type="number" name="sort_order" id="documentTypeSortOrder" min="0" value="<?= old('sort_order', '0') ?>"
                            class="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm text-gray-800 placeholder-gray-400 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
                            placeholder="0">
                ...
                <div class="flex items-end">
                    <div class="flex items-center gap-3 rounded-xl border border-gray-200 bg-gray-50 p-4 w-full">
                        <input type="checkbox" name="is_active" id="documentTypeIsActive" value="1" checked
                            class="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary">
                        <div>
                            <label for="documentTypeIsActive" class="cursor-pointer text-sm font-medium text-gray-800">Set as active</label>
                            <p class="mt-0.5 text-[11px] text-gray-500">Active document types appear in selection lists.</p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="flex items-center justify-end gap-3 border-t border-gray-100 px-6 py-4 bg-gray-50 rounded-b-2xl">
                <button type="button" onclick="closeDocumentTypeModal()"
                ...
                <button type="submit" id="documentTypeSubmitBtn"
...
function syncBadgeColorInputs() {
    ...
    const hexInput = document.getElementById('documentTypeBadgeHex');
...
function resetDocumentTypeForm() {
    ...
    document.getElementById('documentTypeBadgeColor').value = '#2563EB';
...
function validateDocumentTypeForm() {
    let valid = true;
    const name = document.getElementById('documentTypeName');
    const color = document.getElementById('documentTypeBadgeHex');
    const sortOrder = document.getElementById('documentTypeSortOrder');
