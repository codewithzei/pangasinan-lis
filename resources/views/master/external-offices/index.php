...
$error = $error ?? null;
$pageTitle = $pageTitle ?? 'External Offices Management';
$pageSubtitle = $pageSubtitle ?? '';
$totalExternalOffices = $totalExternalOffices ?? 0;
$activeExternalOffices = $activeExternalOffices ?? 0;
$inactiveExternalOffices = $inactiveExternalOffices ?? 0;
...
<div class="space-y-6">
        ...
        <div class="relative px-6 py-8 sm:px-8">
            <div class="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    <button type="button" onclick="openExternalOfficeModal()"
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        <p class="text-sm text-gray-500">Total External Offices</p>
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($totalExternalOffices) ?></p>
                    </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        <p class="text-sm text-gray-500">Active External Offices</p>
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($activeExternalOffices) ?></p>
                    </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        <p class="text-sm text-gray-500">Inactive External Offices</p>
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($inactiveExternalOffices) ?></p>
                    </div>
    ...
    <section class="rounded-2xl border border-gray-200 bg-white">
        <div class="flex flex-col gap-4 border-b border-gray-100 px-6 py-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <h2 class="font-semibold text-gray-900">External Offices List</h2>
                <p class="mt-1 text-xs text-gray-500">
                    <?= $totalRows ?> record<?= $totalRows !== 1 ? 's' : '' ?> found
                </p>
            </div>
            <form method="GET" action="<?= BASE_URL ?>/master/external-offices" class="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div class="relative">
                ...
                <?php if ($search !== '' || $filterStatus !== ''): ?>
                    <a href="<?= BASE_URL ?>/master/external-offices"
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm">
                <thead class="bg-gray-50 text-xs uppercase text-gray-500">
                    <tr>
                        <th class="px-6 py-3 font-medium">External Office</th>
                    ...
                    <?php if (empty($externalOffices)): ?>
                        <tr>
                            <td colspan="5" class="px-6 py-16 text-center">
                                ...
                                <p class="mt-4 text-sm font-medium text-gray-700">No external offices found.</p>
                                <p class="mt-1 text-xs text-gray-500">Create your first external office to populate dropdowns.</p>
                                <button type="button" onclick="openExternalOfficeModal()"
                                    ...
                                    Create External Office
                                </button>
                            </td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($externalOffices as $eo): ?>
                        <tr class="hover:bg-gray-50/50 transition">
                            <td class="px-6 py-4">
                                <div>
                                    <div class="font-medium text-gray-900">
                                        <?= htmlspecialchars($eo['name']) ?>
                                        <?php if (!empty($eo['abbreviation'])): ?>
                                            <span class="ml-2 inline-flex items-center rounded-md bg-indigo-50 px-2 py-0.5 text-[11px] font-semibold text-indigo-700">
                                                <?= htmlspecialchars($eo['abbreviation']) ?>
                                            </span>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    <button type="button" onclick="editExternalOffice(<?= $eo['id'] ?>)"
                                    ...
                                    <button type="button" onclick="toggleStatus(<?= $eo['id'] ?>, '<?= htmlspecialchars($eo['name']) ?>')"
                                        ...
                                        title="<?= $eo['is_active'] ? 'Set Inactive' : 'Set Active' ?>">
                                        <?php if ($eo['is_active']): ?>
                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                  d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"/>
                                    ...
                                    <button type="button" onclick="deleteExternalOffice(<?= $eo['id'] ?>, '<?= htmlspecialchars($eo['name']) ?>')"
    ...
    <div id="externalOfficeModalPanel" class="relative z-10 w-full max-w-xl mx-4 bg-white rounded-2xl shadow-2xl border border-gray-100 transform scale-95 opacity-0 transition-all duration-200 max-h-[90vh] overflow-y-auto">
        <form id="externalOfficeForm" method="POST" action="<?= BASE_URL ?>/master/external-offices/store">
            ...
            <div class="flex items-center justify-between border-b border-gray-100 px-6 py-5">
                <div>
                    <h3 id="externalOfficeModalTitle" class="text-lg font-semibold text-gray-900">Create New External Office</h3>
                    <p class="mt-0.5 text-xs text-gray-500">Fill in the external office details below.</p>
                </div>
                <button type="button" onclick="closeExternalOfficeModal()" class="rounded-lg p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                    <div class="sm:col-span-2">
                        <label class="mb-1.5 block text-xs font-semibold text-gray-700">
                            External Office Name <span class="text-red-500">*</span>
                        </label>
                        <input type="text" name="name" id="externalOfficeName" value="<?= old('name') ?>"
                            ...
                            placeholder="e.g. General Services Office" required>
                    </div>
                    <div>
                        <label class="mb-1.5 block text-xs font-semibold text-gray-700">
                            Abbreviation
                        </label>
                        <input type="text" name="abbreviation" id="externalOfficeAbbreviation" value="<?= old('abbreviation') ?>"
                            class="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm text-gray-800 placeholder-gray-400 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
                            placeholder="e.g. GSO">
                    </div>
                </div>
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="number" name="sort_order" id="sortOrder" min="0" value="<?= old('sort_order', '0') ?>"
                            class="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm text-gray-800 placeholder-gray-400 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
                            placeholder="0">
                        <p class="mt-1 text-[11px] text-gray-500">External offices with lower values appear first in dropdowns.</p>
                    </div>
                    <div class="flex items-end">
                        <div class="flex items-center gap-3 rounded-xl border border-gray-200 bg-gray-50 p-4 w-full">
                            ...
                            <div>
                                ...
                                <p class="text-[11px] text-gray-500 mt-0.5">Active external offices appear in selection dropdowns.</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex items-center justify-end gap-3 border-t border-gray-100 px-6 py-4 bg-gray-50 rounded-b-2xl">
                <button type="button" onclick="closeExternalOfficeModal()"
                ...
                <button type="submit" id="externalOfficeSubmitBtn"
...
function validateExternalOfficeForm() {
    let valid = true;

    const name = document.getElementById('externalOfficeName');
    if (name.value.trim() === '') {
        setFieldError('externalOfficeName', 'External office name is required.');
        valid = false;
    } else clearFieldError('externalOfficeName');

    const abbr = document.getElementById('externalOfficeAbbreviation');
    if (abbr.value.length > 50) {
        setFieldError('externalOfficeAbbreviation', 'Abbreviation too long (max 50 chars).');
        valid = false;
    } else clearFieldError('externalOfficeAbbreviation');
...
function attachExternalOfficeValidation() {
    const fields = ['externalOfficeName', 'externalOfficeAbbreviation', 'sortOrder'];
    fields.forEach(id => {
        const el = document.getElementById(id);
        if (!el) return;
        el.addEventListener('input', validateExternalOfficeForm);
        el.addEventListener('blur', validateExternalOfficeForm);
        el.addEventListener('change', validateExternalOfficeForm);
    });
    document.getElementById('externalOfficeForm')?.addEventListener('submit', (e) => {
        if (!validateExternalOfficeForm()) {
            e.preventDefault();
...
// External Office Modal
let externalOfficeModalOpen = false;
function resetExternalOfficeForm() {
    const form = document.getElementById('externalOfficeForm');
    form.reset();
    document.getElementById('externalOfficeId').value = '';
    document.getElementById('externalOfficeName').value = '';
    document.getElementById('externalOfficeAbbreviation').value = '';
    document.getElementById('sortOrder').value = '0';
    document.getElementById('isActive').checked = true;
    form.action = '<?= BASE_URL ?>/master/external-offices/store';
    document.getElementById('externalOfficeModalTitle').textContent = 'Create New External Office';
    document.getElementById('externalOfficeSubmitBtn').innerHTML = `
        <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
        </svg>
        Save
    `;
    ['externalOfficeName', 'externalOfficeAbbreviation', 'sortOrder'].forEach(clearFieldError);
    attachExternalOfficeValidation();
}
function openExternalOfficeModal() {
    ...
    setTimeout(() => document.getElementById('externalOfficeName').focus(), 220);
}
function closeExternalOfficeModal() {
    if (!externalOfficeModalOpen) return;
...
async function editExternalOffice(id) {
    try {
        const res = await fetch(`<?= BASE_URL ?>/master/external-offices/edit?id=${id}`);
        const data = await res.json();
        if (!data.success) {
            showToast(data.message || 'External office not found.', 'error');
            return;
        }
        const eo = data.data;
        document.getElementById('externalOfficeId').value = eo.id;
        document.getElementById('externalOfficeName').value = eo.name;
        document.getElementById('externalOfficeAbbreviation').value = eo.abbreviation || '';
        document.getElementById('sortOrder').value = eo.sort_order ?? 0;
        document.getElementById('isActive').checked = !!eo.is_active;
        document.getElementById('externalOfficeForm').action = '<?= BASE_URL ?>/master/external-offices/update';
        document.getElementById('externalOfficeModalTitle').textContent = 'Edit External Office';
        document.getElementById('externalOfficeSubmitBtn').innerHTML = `
            <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        showToast('Failed to load external office data.', 'error');
    }
}
    ...
    openConfirmModal({
        ...
        onConfirm: async () => {
            try {
                ...
                fd.append('id', id);
                const res = await fetch('<?= BASE_URL ?>/master/external-offices/destroy', { method: 'POST', body: fd });
                const data = await res.json();
...
async function toggleStatus(id, name) {
    openConfirmModal({
        title: 'Toggle Status',
        message: `This will toggle the active status of <strong class="text-gray-800">${name}</strong>.<br><span class="text-xs text-gray-500">Inactive external offices are hidden from selection dropdowns.</span>`,
        confirmText: 'Proceed',
        type: 'info',
        onConfirm: async () => {
            try {
                const fd = new FormData();
                fd.append('id', id);
                const res = await fetch('<?= BASE_URL ?>/master/external-offices/toggle-status', { method: 'POST', body: fd });
                const data = await res.json();
                showToast(data.message, data.success ? 'success' : 'error');
