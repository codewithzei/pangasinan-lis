...
$error = $error ?? null;
$pageTitle = $pageTitle ?? 'Routing Options Management';
$pageSubtitle = $pageSubtitle ?? '';
$totalRoutingOptions = $totalRoutingOptions ?? 0;
$activeRoutingOptions = $activeRoutingOptions ?? 0;
$inactiveRoutingOptions = $inactiveRoutingOptions ?? 0;
...
<div class="space-y-6">
    ...
    <section class="overflow-hidden rounded-2xl bg-linear-to-br from-primary via-primary to-blue-700 shadow-md">
        <div class="relative px-6 py-8 sm:px-8">
            <div class="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                <div class="max-w-3xl">
                    <p class="text-sm font-medium text-blue-100">ROUTING OPTION MASTER DATA</p>
                ...
                <div class="flex flex-wrap items-center gap-3">
                    <button type="button" onclick="openRoutingOptionModal()"
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        <p class="text-sm text-gray-500">Total Routing Options</p>
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($totalRoutingOptions) ?></p>
                    </div>
                ...
                <div class="mt-4 flex items-center gap-2 text-xs">
                    <span class="rounded-full bg-blue-50 px-2 py-1 font-medium text-primary">All routing stages</span>
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        <p class="text-sm text-gray-500">Active Routing Options</p>
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($activeRoutingOptions) ?></p>
                    </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        <p class="text-sm text-gray-500">Inactive Routing Options</p>
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($inactiveRoutingOptions) ?></p>
                    </div>
    ...
    <section class="rounded-2xl border border-gray-200 bg-white">
        <div class="flex flex-col gap-4 border-b border-gray-100 px-6 py-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <h2 class="font-semibold text-gray-900">Routing Options List</h2>
                <p class="mt-1 text-xs text-gray-500">
                    <?= $totalRows ?> record<?= $totalRows !== 1 ? 's' : '' ?> found
                </p>
            </div>
            <form method="GET" action="<?= BASE_URL ?>/master/routing-options" class="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div class="relative">
                    <svg class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                    </svg>
                    <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Search routing option..."
                ...
                <?php if ($search !== '' || $filterStatus !== ''): ?>
                    <a href="<?= BASE_URL ?>/master/routing-options"
                    ...
                    <?php if (empty($routingOptions)): ?>
                        <tr>
                            <td colspan="4" class="px-6 py-16 text-center">
                                ...
                                <p class="mt-4 text-sm font-medium text-gray-700">No routing options found.</p>
                                <p class="mt-1 text-xs text-gray-500">Create a workflow route to organize document movement.</p>
                                <button type="button" onclick="openRoutingOptionModal()"
                    ...
                    <?php else: ?>
                        <?php foreach ($routingOptions as $routingOption): ?>
                            <tr class="hover:bg-gray-50/50 transition">
                                <td class="px-6 py-4">
                                    <div>
                                        <div class="font-medium text-gray-900"><?= htmlspecialchars($routingOption['name']) ?></div>
                                        <?php if (!empty($routingOption['description'])): ?>
                                            <div class="mt-0.5 text-xs text-gray-500 line-clamp-1 max-w-xl"><?= htmlspecialchars($routingOption['description']) ?></div>
                                        <?php endif; ?>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center rounded-lg bg-gray-50 border border-gray-200 px-2.5 py-1 text-xs font-medium text-gray-600">
                                        #<?= (int)$routingOption['sort_order'] ?>
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <?php if ($routingOption['is_active']): ?>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        <button type="button" onclick="editRoutingOption(<?= $routingOption['id'] ?>)"
                                        ...
                                        <button type="button" onclick="toggleStatus(<?= $routingOption['id'] ?>, '<?= htmlspecialchars($routingOption['name']) ?>')"
                                            ...
                                            title="<?= $routingOption['is_active'] ? 'Set Inactive' : 'Set Active' ?>">
                                            <?php if ($routingOption['is_active']): ?>
                                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <button type="button" onclick="deleteRoutingOption(<?= $routingOption['id'] ?>, '<?= htmlspecialchars($routingOption['name']) ?>')"
...
<div id="routingOptionModal" class="fixed inset-0 z-100 hidden items-center justify-center" role="dialog" aria-modal="true">
    <div id="routingOptionModalBackdrop" class="absolute inset-0 bg-gray-900/50 backdrop-blur-sm transition-opacity opacity-0"></div>
    <div id="routingOptionModalPanel" class="relative z-10 w-full max-w-xl mx-4 bg-white rounded-2xl shadow-2xl border border-gray-100 transform scale-95 opacity-0 transition-all duration-200 max-h-[90vh] overflow-y-auto">
        <form id="routingOptionForm" method="POST" action="<?= BASE_URL ?>/master/routing-options/store">
            <input type="hidden" name="id" id="routingOptionId" value="">
            <div class="flex items-center justify-between border-b border-gray-100 px-6 py-5">
                <div>
                    <h3 id="routingOptionModalTitle" class="text-lg font-semibold text-gray-900">Create New Routing Option</h3>
                    <p class="mt-0.5 text-xs text-gray-500">Fill in the routing option details below.</p>
                </div>
                <button type="button" onclick="closeRoutingOptionModal()" class="rounded-lg p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="p-6 space-y-5">
                <div>
                    <label class="mb-1.5 block text-xs font-semibold text-gray-700">
                        Routing Option Name <span class="text-red-500">*</span>
                    </label>
                    <input type="text" name="name" id="routingOptionName" value="<?= old('name') ?>"
                        ...
                        placeholder="e.g. Receiving Staff" required>
                </div>

                <div>
                    <label class="mb-1.5 block text-xs font-semibold text-gray-700">Description / Notes</label>
                    <textarea name="description" id="routingOptionDescription" rows="4"
                        ...
                        placeholder="Optional description for this routing stage..."><?= old('description') ?></textarea>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" id="routingOptionSortOrder" min="0" value="<?= old('sort_order', '0') ?>"
                        class="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm text-gray-800 placeholder-gray-400 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
                        placeholder="0">
                    <p class="mt-1 text-[11px] text-gray-500">Lower values appear first in the routing workflow.</p>
                </div>

                <div class="flex items-end">
                    <div class="flex items-center gap-3 rounded-xl border border-gray-200 bg-gray-50 p-4 w-full">
                        <input type="checkbox" name="is_active" id="routingOptionIsActive" value="1" checked
                            class="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary">
                        <div>
                            <label for="routingOptionIsActive" class="cursor-pointer text-sm font-medium text-gray-800">Set as active</label>
                            <p class="mt-0.5 text-[11px] text-gray-500">Active routing options appear in selection lists.</p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="flex items-center justify-end gap-3 border-t border-gray-100 px-6 py-4 bg-gray-50 rounded-b-2xl">
                <button type="button" onclick="closeRoutingOptionModal()"
                ...
                <button type="submit" id="routingOptionSubmitBtn"
...
<script>
const BASE = '<?= BASE_URL ?>';
let routingOptionModalOpen = false;
...
function resetRoutingOptionForm() {
    const form = document.getElementById('routingOptionForm');
    form.reset();
    document.getElementById('routingOptionId').value = '';
    document.getElementById('routingOptionName').value = '';
    document.getElementById('routingOptionDescription').value = '';
    document.getElementById('routingOptionSortOrder').value = '0';
    document.getElementById('routingOptionIsActive').checked = true;
    form.action = `${BASE}/master/routing-options/store`;
    document.getElementById('routingOptionModalTitle').textContent = 'Create New Routing Option';
    document.getElementById('routingOptionSubmitBtn').innerHTML = `
        <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
        </svg>
        Save
...
function openRoutingOptionModal() {
    if (routingOptionModalOpen) return;
    resetRoutingOptionForm();
    routingOptionModalOpen = true;

    const modal = document.getElementById('routingOptionModal');
    const panel = document.getElementById('routingOptionModalPanel');
    const backdrop = document.getElementById('routingOptionModalBackdrop');
    ...
    setTimeout(() => document.getElementById('routingOptionName').focus(), 220);
}

function closeRoutingOptionModal() {
    if (!routingOptionModalOpen) return;
    routingOptionModalOpen = false;
...
document.getElementById('routingOptionModalBackdrop')?.addEventListener('click', closeRoutingOptionModal);
...
async function editRoutingOption(id) {
    try {
        const response = await fetch(`${BASE}/master/routing-options/edit?id=${id}`);
        const result = await response.json();
        if (!result.success) {
            showToast(result.message || 'Unable to load routing option.', 'error');
            return;
        }

        const item = result.data;
        const form = document.getElementById('routingOptionForm');
        document.getElementById('routingOptionId').value = item.id;
        document.getElementById('routingOptionName').value = item.name || '';
        document.getElementById('routingOptionDescription').value = item.description || '';
        document.getElementById('routingOptionSortOrder').value = item.sort_order ?? 0;
        document.getElementById('routingOptionIsActive').checked = Number(item.is_active) === 1;
        form.action = `${BASE}/master/routing-options/update`;
        document.getElementById('routingOptionModalTitle').textContent = 'Edit Routing Option';
        document.getElementById('routingOptionSubmitBtn').innerHTML = `
            <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
            </svg>
        ...
        showToast('Failed to load routing option data.', 'error');
    }
}

function deleteRoutingOption(id, name) {
    openConfirmModal({
        title: 'Delete Routing Option',
        ...
        onConfirm: async () => {
            ...
            const response = await fetch(`${BASE}/master/routing-options/destroy`, {
                method: 'POST',
                body: formData,
            });
            const result = await response.json();
            showToast(result.message || (result.success ? 'Routing option deleted successfully.' : 'Failed to delete routing option.'), result.success ? 'success' : 'error');
...
function toggleStatus(id, name) {
    openConfirmModal({
        title: 'Toggle Status',
        message: `This will toggle the active status of <strong class="text-gray-800">${name}</strong>.<br><span class="text-xs text-gray-500">Inactive routing options are hidden from selection dropdowns.</span>`,
        confirmText: 'Proceed',
        type: 'info',
        onConfirm: async () => {
            const formData = new FormData();
            formData.append('id', id);
            const response = await fetch(`${BASE}/master/routing-options/toggle-status`, {
                method: 'POST',
                body: formData,
            });
            const result = await response.json();
            showToast(result.message || (result.success ? 'Status updated successfully.' : 'Unable to update routing option.'), result.success ? 'success' : 'error');
            if (result.success) setTimeout(() => location.reload(), 800);
        }
    });
}

function validateRoutingOptionForm() {
    let valid = true;
    const name = document.getElementById('routingOptionName');
    const sortOrder = document.getElementById('routingOptionSortOrder');
...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
        if (routingOptionModalOpen) closeRoutingOptionModal();
