...
$error = $error ?? null;
$pageTitle = $pageTitle ?? 'Municipalities & Cities Management';
$pageSubtitle = $pageSubtitle ?? '';
$totalMunicipalities = $totalMunicipalities ?? 0;
$activeMunicipalities = $activeMunicipalities ?? 0;
$inactiveMunicipalities = $inactiveMunicipalities ?? 0;
...
<div class="space-y-6">
        ...
        <div class="relative px-6 py-8 sm:px-8">
            <div class="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    <button type="button" onclick="openMuniCityModal()"
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        <p class="text-sm text-gray-500">Total Municipalities</p>
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($totalMunicipalities) ?></p>
                    </div>
                    <div class="flex h-11 w-11 items-center justify-center rounded-xl bg-blue-50 text-blue-600">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-building2-icon lucide-building-2"><path d="M6 22V4a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v18"/><path d="M6 5h12"/><path d="M6 10h12"/><path d="M10 15h4"/><path d="M10 19h4"/></svg>
                    </div>
                </div>
                <div class="mt-4 flex items-center gap-2 text-xs">
                    <span class="rounded-full bg-blue-50 px-2 py-1 font-medium text-blue-600">All LGUs</span>
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        ...
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($activeMunicipalities) ?></p>
                    </div>
                ...
                <div class="mt-4 flex items-center gap-2 text-xs">
                    <span class="rounded-full bg-emerald-50 px-2 py-1 font-medium text-emerald-600">Available in forms</span>
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        ...
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($inactiveMunicipalities) ?></p>
                    </div>
    ...
    <section class="rounded-2xl border border-gray-200 bg-white">
        <div class="flex flex-col gap-4 border-b border-gray-100 px-6 py-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <h2 class="font-semibold text-gray-900">Municipalities &amp; Cities List</h2>
                <p class="mt-1 text-xs text-gray-500">
                    <?= $totalRows ?> record<?= $totalRows !== 1 ? 's' : '' ?> found
                </p>
            </div>
            <form method="GET" action="<?= BASE_URL ?>/master/municipalities" class="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div class="relative">
                    ...
                    <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Search municipality/city..."
                ...
                <select name="district_id" class="rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm text-gray-800 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20">
                    ...
                    <?php foreach ($districtOptions as $district): ?>
                        <option value="<?= (int)$district['id'] ?>" <?= (string)$districtFilter === (string)$district['id'] ? 'selected' : '' ?>>
                            <?= htmlspecialchars($district['name']) ?>
                            <?php if (!empty($district['district_number'])): ?>
                                (District <?= (int)$district['district_number'] ?>)
                            <?php endif; ?>
                        </option>
                    <?php endforeach; ?>
                </select>
                ...
                <?php if ($search !== '' || $filterStatus !== '' || $districtFilter !== ''): ?>
                    <a href="<?= BASE_URL ?>/master/municipalities"
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm">
                <thead class="bg-gray-50 text-xs uppercase text-gray-500">
                    <tr>
                        <th class="px-6 py-3 font-medium"> Municipality / City </th>
                ...
                <tbody class="divide-y divide-gray-100">
                    <?php if (empty($municipalities)): ?>
                        <tr>
                            <td colspan="6" class="px-6 py-16 text-center">
                                ...
                                <p class="mt-4 text-sm font-medium text-gray-700">No municipalities or cities found.</p>
                                <p class="mt-1 text-xs text-gray-500">Create your first entry and assign it to a district.</p>
                                <button type="button" onclick="openMuniCityModal()"
                    ...
                    <?php else: ?>
                        ...
                        <tr class="hover:bg-gray-50/50 transition">
                            <td class="px-6 py-4">
                                <div>
                                    <div class="font-medium text-gray-900"><?= htmlspecialchars($m['name']) ?></div>
                                    <?php if (!empty($m['description'])): ?>
                                        <div class="mt-0.5 text-xs text-gray-500 line-clamp-1 max-w-lg"><?= htmlspecialchars($m['description']) ?></div>
                                    <?php endif; ?>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <?php if (!empty($m['district_name'])): ?>
                                    <span class="inline-flex items-center rounded-lg bg-emerald-50 border border-emerald-200 px-2.5 py-1 text-xs font-semibold text-emerald-700">
                                        <?= htmlspecialchars($m['district_name']) ?>
                                    </span>
                                <?php else: ?>
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center rounded-lg bg-blue-50 border border-blue-200 px-2.5 py-1 text-xs font-medium text-blue-700">
                                    <?= htmlspecialchars($m['type'] ?: 'Municipality') ?>
                                </span>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    <button type="button" onclick="editMuniCity(<?= $m['id'] ?>)"
                                    ...
                                    <button type="button" onclick="toggleStatus(<?= $m['id'] ?>, '<?= htmlspecialchars($m['name']) ?>')"
                                        ...
                                        title="<?= $m['is_active'] ? 'Set Inactive' : 'Set Active' ?>">
                                        <?php if ($m['is_active']): ?>
                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                  d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"/>
                                    ...
                                    <button type="button" onclick="deleteMuniCity(<?= $m['id'] ?>, '<?= htmlspecialchars($m['name']) ?>')"
        ...
        <div class="flex flex-col items-center justify-between gap-3 border-t border-gray-100 px-6 py-4 sm:flex-row">
            ...
            <div class="flex items-center gap-1">
                ...
                if ($districtFilter !== '') $query['district_id'] = $districtFilter;
    ...
    <div id="muniCityModalPanel" class="relative z-10 w-full max-w-xl mx-4 bg-white rounded-2xl shadow-2xl border border-gray-100 transform scale-95 opacity-0 transition-all duration-200 max-h-[90vh] overflow-y-auto">
        <form id="muniCityForm" method="POST" action="<?= BASE_URL ?>/master/municipalities/store">
            ...
            <div class="flex items-center justify-between border-b border-gray-100 px-6 py-5">
                <div>
                    <h3 id="muniCityModalTitle" class="text-lg font-semibold text-gray-900">Create New Municipality/City</h3>
                    <p class="mt-0.5 text-xs text-gray-500">Fill in the local government unit details below.</p>
                </div>
                <button type="button" onclick="closeMuniCityModal()" class="rounded-lg p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="p-6 space-y-5">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <select name="district_id" id="muniCityDistrictId" class="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm text-gray-800 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20" required>
                    ...
                    <div>
                        ...
                        <select name="type" id="muniCityType" class="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm text-gray-800 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20" required>
                ...
                <div>
                    <label class="mb-1.5 block text-xs font-semibold text-gray-700">
                        Municipality / City Name <span class="text-red-500">*</span>
                    </label>
                    <input type="text" name="name" id="muniCityName" value="<?= old('name') ?>"
                        ...
                        placeholder="e.g. Dagupan City" required>
                </div>

                <div>
                    <label class="mb-1.5 block text-xs font-semibold text-gray-700">Description / Notes</label>
                    <textarea name="description" id="muniCityDescription" rows="3"
                        ...
                        placeholder="Optional description or note for this LGU..."><?= old('description') ?></textarea>
                </div>

                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        <label class="mb-1.5 block text-xs font-semibold text-gray-700">Sort Order</label>
                        <input type="number" name="sort_order" id="muniCitySortOrder" min="0" value="<?= old('sort_order', '0') ?>"
                            class="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm text-gray-800 placeholder-gray-400 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
                            placeholder="0">
                    </div>
                    <div class="flex items-end">
                        <div class="flex items-center gap-3 rounded-xl border border-gray-200 bg-gray-50 p-4 w-full">
                            <input type="checkbox" name="is_active" id="muniCityIsActive" value="1" checked
                                class="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary">
                            <div>
                                <label for="muniCityIsActive" class="cursor-pointer text-sm font-medium text-gray-800">Set as active</label>
                                <p class="text-[11px] text-gray-500 mt-0.5">Active LGUs appear in selection dropdowns.</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex items-center justify-end gap-3 border-t border-gray-100 px-6 py-4 bg-gray-50 rounded-b-2xl">
                <button type="button" onclick="closeMuniCityModal()"
                ...
                <button type="submit" id="muniCitySubmitBtn"
...
let muniCityModalOpen = false;
let confirmModalOpen = false;
let pendingAction = null;

function resetMuniCityForm() {
    const form = document.getElementById('muniCityForm');
    form.reset();
    document.getElementById('muniCityId').value = '';
    document.getElementById('muniCityDistrictId').value = '';
    document.getElementById('muniCityType').value = 'Municipality';
    document.getElementById('muniCityName').value = '';
    document.getElementById('muniCityDescription').value = '';
    document.getElementById('muniCitySortOrder').value = '0';
    document.getElementById('muniCityIsActive').checked = true;
    form.action = '<?= BASE_URL ?>/master/municipalities/store';
    document.getElementById('muniCityModalTitle').textContent = 'Create New Municipality/City';
    document.getElementById('muniCitySubmitBtn').innerHTML = `
        <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
        </svg>
        Save
...
function openMuniCityModal() {
    ...
    setTimeout(() => document.getElementById('muniCityName').focus(), 220);
}

function closeMuniCityModal() {
    if (!muniCityModalOpen) return;
...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
        if (muniCityModalOpen) closeMuniCityModal();
        if (confirmModalOpen) closeConfirmModal();
    }
});

async function editMuniCity(id) {
    try {
        const res = await fetch(`<?= BASE_URL ?>/master/municipalities/edit?id=${id}`);
        const data = await res.json();
        if (!data.success) {
            showToast(data.message || 'Municipality/city not found.', 'error');
            return;
        }
        const d = data.data;
        document.getElementById('muniCityId').value = d.id;
        document.getElementById('muniCityDistrictId').value = d.district_id ?? '';
        document.getElementById('muniCityType').value = d.type || 'Municipality';
        document.getElementById('muniCityName').value = d.name || '';
        document.getElementById('muniCityDescription').value = d.description || '';
        document.getElementById('muniCitySortOrder').value = d.sort_order ?? 0;
        document.getElementById('muniCityIsActive').checked = !!d.is_active;
        document.getElementById('muniCityForm').action = '<?= BASE_URL ?>/master/municipalities/update';
        document.getElementById('muniCityModalTitle').textContent = 'Edit Municipality/City';
        document.getElementById('muniCitySubmitBtn').innerHTML = `
            <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        showToast('Failed to load municipality/city data.', 'error');
    }
}
...
async function deleteMuniCity(id, name) {
    openConfirmModal({
        title: 'Delete Municipality/City',
        ...
        onConfirm: async () => {
            try {
                ...
                fd.append('id', id);
                const res = await fetch('<?= BASE_URL ?>/master/municipalities/destroy', { method: 'POST', body: fd });
                const data = await res.json();
...
async function toggleStatus(id, name) {
    openConfirmModal({
        title: 'Toggle Status',
        message: `This will toggle the active status of <strong class="text-gray-800">${name}</strong>.<br><span class="text-xs text-gray-500">Inactive LGUs are hidden from selection dropdowns.</span>`,
        confirmText: 'Proceed',
        type: 'info',
        onConfirm: async () => {
            try {
                const fd = new FormData();
                fd.append('id', id);
                const res = await fetch('<?= BASE_URL ?>/master/municipalities/toggle-status', { method: 'POST', body: fd });
                const data = await res.json();
                showToast(data.message, data.success ? 'success' : 'error');
