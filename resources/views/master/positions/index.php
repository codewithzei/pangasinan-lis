...
$error = $error ?? null;
$pageTitle = $pageTitle ?? 'Positions Management';
$pageSubtitle = $pageSubtitle ?? '';
$totalPositions = $totalPositions ?? 0;
$activePositions = $activePositions ?? 0;
$inactivePositions = $inactivePositions ?? 0;
...
<div class="space-y-6">
    ...
    <section class="overflow-hidden rounded-2xl bg-gradient-to-br from-indigo-700 via-primary to-blue-700 shadow-md">
        <div class="relative px-6 py-8 sm:px-8">
            <div class="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                <div class="max-w-3xl">
                    <p class="text-sm font-medium text-blue-100">POSITION MASTER DATA</p>
    ...
    <section>
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        ...
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($activePositions) ?></p>
                    </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        <p class="text-sm text-gray-500">Inactive Positions</p>
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($inactivePositions) ?></p>
                    </div>
    ...
    <section class="rounded-2xl border border-gray-200 bg-white">
        <div class="flex flex-col gap-4 border-b border-gray-100 px-6 py-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <h2 class="font-semibold text-gray-900">Positions List</h2>
                <p class="mt-1 text-xs text-gray-500">
                    <?= $totalRows ?> record<?= $totalRows !== 1 ? 's' : '' ?> found
                </p>
            </div>
            <form method="GET" action="<?= BASE_URL ?>/master/positions" class="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div class="relative">
                ...
                <?php if ($search !== '' || $filterStatus !== ''): ?>
                    <a href="<?= BASE_URL ?>/master/positions"
                        ...
                        <tr class="hover:bg-gray-50/50 transition">
                            <td class="px-6 py-4">
                                <div>
                                    <div class="font-medium text-gray-900">
                                        ...
                                        <?php if (!empty($p['abbreviation'])): ?>
                                            <span class="ml-2 inline-flex items-center rounded-md bg-indigo-50 px-2 py-0.5 text-[11px] font-semibold text-indigo-700">
                                                <?= htmlspecialchars($p['abbreviation']) ?>
                                            </span>
                                        <?php endif; ?>
                                    </div>
                                    <?php if (!empty($p['description'])): ?>
                                        <div class="mt-0.5 text-xs text-gray-500 line-clamp-1 max-w-lg"><?= htmlspecialchars($p['description']) ?></div>
                                    <?php endif; ?>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    ...
                                    <button type="button" onclick="toggleStatus(<?= $p['id'] ?>, '<?= htmlspecialchars($p['name']) ?>')"
...
<div id="positionModal" class="fixed inset-0 z-[100] hidden items-center justify-center" role="dialog" aria-modal="true">
    <div id="positionModalBackdrop" class="absolute inset-0 bg-gray-900/50 backdrop-blur-sm transition-opacity opacity-0"></div>
    <div id="positionModalPanel" class="relative z-10 w-full max-w-xl mx-4 bg-white rounded-2xl shadow-2xl border border-gray-100 transform scale-95 opacity-0 transition-all duration-200 max-h-[90vh] overflow-y-auto">
        <form id="positionForm" method="POST" action="<?= BASE_URL ?>/master/positions/store">
            <input type="hidden" name="id" id="positionId" value="">
            <div class="flex items-center justify-between border-b border-gray-100 px-6 py-5">
                <div>
                    <h3 id="positionModalTitle" class="text-lg font-semibold text-gray-900">Create New Position</h3>
                    <p class="mt-0.5 text-xs text-gray-500">Fill in the position details below.</p>
                </div>
                <button type="button" onclick="closePositionModal()" class="rounded-lg p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="p-6 space-y-5">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="name" id="positionName" value="<?= old('name') ?>"
                            ...
                            placeholder="e.g. Board Member" required>
                    </div>
                    <div>
                        <label class="mb-1.5 block text-xs font-semibold text-gray-700">
                            Abbreviation
                        </label>
                        <input type="text" name="abbreviation" id="positionAbbreviation" value="<?= old('abbreviation') ?>"
                            class="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm text-gray-800 placeholder-gray-400 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
                            placeholder="e.g. BM">
                    </div>
                </div>
                <div>
                    <label class="mb-1.5 block text-xs font-semibold text-gray-700">Description / Role</label>
                    <textarea name="description" id="positionDescription" rows="3"
                        ...
                        placeholder="Optional description or notes about this position..."><?= old('description') ?></textarea>
                </div>
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="number" name="sort_order" id="sortOrder" min="0" value="<?= old('sort_order', '0') ?>"
                            class="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm text-gray-800 placeholder-gray-400 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
                            placeholder="0">
                        <p class="mt-1 text-[11px] text-gray-500">Positions with lower values appear first in dropdowns.</p>
                    </div>
                    <div class="flex items-end">
                        <div class="flex items-center gap-3 rounded-xl border border-gray-200 bg-gray-50 p-4 w-full">
                            ...
                            <div>
                                ...
                                <p class="text-[11px] text-gray-500 mt-0.5">Active positions appear in selection dropdowns.</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex items-center justify-end gap-3 border-t border-gray-100 px-6 py-4 bg-gray-50 rounded-b-2xl">
                ...
                <button type="submit" id="positionSubmitBtn"
...
function validatePositionForm() {
    ...
    const name = document.getElementById('positionName');
    if (name.value.trim() === '') {
        setFieldError('positionName', 'Position name is required.');
        valid = false;
    } else clearFieldError('positionName');

    const abbr = document.getElementById('positionAbbreviation');
    if (abbr.value.length > 50) {
        setFieldError('positionAbbreviation', 'Abbreviation too long (max 50 chars).');
        valid = false;
    } else clearFieldError('positionAbbreviation');
...
function attachPositionValidation() {
    const fields = ['positionName', 'positionAbbreviation', 'sortOrder'];
    fields.forEach(id => {
        const el = document.getElementById(id);
        if (!el) return;
        el.addEventListener('input', validatePositionForm);
        el.addEventListener('blur', validatePositionForm);
        el.addEventListener('change', validatePositionForm);
    });
...
function resetPositionForm() {
    ...
    document.getElementById('positionAbbreviation').value = '';
    document.getElementById('positionDescription').value = '';
    document.getElementById('sortOrder').value = '0';
    document.getElementById('isActive').checked = true;
    form.action = '<?= BASE_URL ?>/master/positions/store';
    document.getElementById('positionModalTitle').textContent = 'Create New Position';
    document.getElementById('positionSubmitBtn').innerHTML = `
        <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
        </svg>
        Save Position
...
function openPositionModal() {
    ...
    const b = document.getElementById('positionModalBackdrop');
    const p = document.getElementById('positionModalPanel');
    ...
    setTimeout(() => document.getElementById('positionName').focus(), 220);
}
...
document.getElementById('positionModalBackdrop')?.addEventListener('click', closePositionModal);
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
        if (positionModalOpen) closePositionModal();
...
async function editPosition(id) {
    try {
        const res = await fetch(`<?= BASE_URL ?>/master/positions/edit?id=${id}`);
        const data = await res.json();
        if (!data.success) {
            showToast(data.message || 'Position not found.', 'error');
            return;
        }
        const p = data.data;
        document.getElementById('positionId').value = p.id;
        document.getElementById('positionName').value = p.name;
        document.getElementById('positionAbbreviation').value = p.abbreviation || '';
        document.getElementById('positionDescription').value = p.description || '';
        document.getElementById('sortOrder').value = p.sort_order ?? 0;
        document.getElementById('isActive').checked = !!p.is_active;
        document.getElementById('positionForm').action = '<?= BASE_URL ?>/master/positions/update';
        ...
        const m = document.getElementById('positionModal');
        const b = document.getElementById('positionModalBackdrop');
        const pn = document.getElementById('positionModalPanel');
        m.classList.remove('hidden');
        m.classList.add('flex');
        requestAnimationFrame(() => {
            b.classList.remove('opacity-0');
    ...
    openConfirmModal({
        ...
        onConfirm: async () => {
            try {
                ...
                fd.append('id', id);
                const res = await fetch('<?= BASE_URL ?>/master/positions/destroy', { method: 'POST', body: fd });
                const data = await res.json();
...
async function toggleStatus(id, name) {
    openConfirmModal({
        title: 'Toggle Status',
        message: `This will toggle the active status of <strong class="text-gray-800">${name}</strong>.<br><span class="text-xs text-gray-500">Inactive positions are hidden from selection dropdowns.</span>`,
        confirmText: 'Proceed',
        type: 'info',
        onConfirm: async () => {
            try {
                const fd = new FormData();
                fd.append('id', id);
                const res = await fetch('<?= BASE_URL ?>/master/positions/toggle-status', { method: 'POST', body: fd });
                const data = await res.json();
                showToast(data.message, data.success ? 'success' : 'error');
