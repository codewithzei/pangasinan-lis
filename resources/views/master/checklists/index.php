...
$error = $error ?? null;
$pageTitle = $pageTitle ?? 'Checklist/Requirements Management';
$pageSubtitle = $pageSubtitle ?? '';
$totalChecklists = $totalChecklists ?? 0;
$activeChecklists = $activeChecklists ?? 0;
$inactiveChecklists = $inactiveChecklists ?? 0;
...
<div class="space-y-6">
    ...
    <section class="overflow-hidden rounded-2xl bg-linear-to-br from-primary via-primary to-blue-700 shadow-md">
        <div class="relative px-6 py-8 sm:px-8">
            <div class="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                <div class="max-w-3xl">
                    <p class="text-sm font-medium text-blue-100">CHECKLIST & REQUIREMENTS MASTER DATA</p>
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        <p class="text-sm text-gray-500">Total Checklist Items</p>
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($totalChecklists) ?></p>
                    </div>
                ...
                <div class="mt-4 flex items-center gap-2 text-xs">
                    <span class="rounded-full bg-blue-50 px-2 py-1 font-medium text-primary">All requirements</span>
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        ...
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($activeChecklists) ?></p>
                    </div>
                ...
                <div class="mt-4 flex items-center gap-2 text-xs">
                    <span class="rounded-full bg-emerald-50 px-2 py-1 font-medium text-emerald-600">Currently in use</span>
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        <p class="text-sm text-gray-500">Inactive Items</p>
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($inactiveChecklists) ?></p>
                    </div>
                ...
                <div class="mt-4 flex items-center gap-2 text-xs">
                    <span class="rounded-full bg-gray-100 px-2 py-1 font-medium text-gray-600">Not currently used</span>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="rounded-2xl border border-gray-200 bg-white">
        <div class="flex flex-col gap-4 border-b border-gray-100 px-6 py-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <h2 class="font-semibold text-gray-900">Checklist Items List</h2>
                <p class="mt-1 text-xs text-gray-500">
                    <?= $totalRows ?> record<?= $totalRows !== 1 ? 's' : '' ?> found
                </p>
            </div>
            <form method="GET" action="<?= BASE_URL ?>/master/checklists" class="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div class="relative">
                    <svg class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                    </svg>
                    <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Search checklist item..."
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm">
                <thead class="bg-gray-50 text-xs uppercase text-gray-500">
                    <tr>
                        <th class="px-6 py-3 font-medium">Checklist Item</th>
                        <th class="px-6 py-3 font-medium">Document Types</th>
                    ...
                    <?php if (empty($checklists)): ?>
                        <tr>
                            <td colspan="5" class="px-6 py-16 text-center">
                                ...
                                <p class="mt-4 text-sm font-medium text-gray-700">No checklist items found.</p>
                                <p class="mt-1 text-xs text-gray-500">Create a new checklist requirement to get started.</p>
                                <button type="button" onclick="openChecklistModal()"
                    ...
                    <?php else: ?>
                        <?php foreach ($checklists as $checklist): ?>
                            <tr class="hover:bg-gray-50/50 transition">
                                <td class="px-6 py-4">
                                    <div>
                                        <div class="font-medium text-gray-900"><?= htmlspecialchars($checklist['name']) ?></div>
                                        <?php if (!empty($checklist['description'])): ?>
                                            <div class="mt-0.5 text-xs text-gray-500 line-clamp-1 max-w-lg"><?= htmlspecialchars($checklist['description']) ?></div>
                                        <?php endif; ?>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <?php if ((int)$checklist['document_type_count'] > 0): ?>
                                        <a href="<?= BASE_URL ?>/master/checklists/show?id=<?= $checklist['id'] ?>"
                                           class="inline-flex items-center gap-1.5 rounded-full border border-blue-200 bg-blue-50 px-2.5 py-0.5 text-xs font-medium text-blue-700 hover:bg-blue-100 transition">
                                            <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                            </svg>
                                            <?= (int)$checklist['document_type_count'] ?> assigned
                                        </a>
                                    <?php else: ?>
                                        <a href="<?= BASE_URL ?>/master/checklists/show?id=<?= $checklist['id'] ?>"
                                           class="inline-flex items-center gap-1.5 rounded-full border border-gray-200 bg-gray-50 px-2.5 py-0.5 text-xs font-medium text-gray-500 hover:bg-gray-100 transition">
                                            None assigned
                                        </a>
                                    <?php endif; ?>
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        ...
                                        <button type="button" onclick="editChecklist(<?= $checklist['id'] ?>)"
                                        ...
                                        <button type="button" onclick="toggleStatus(<?= $checklist['id'] ?>, '<?= htmlspecialchars($checklist['name']) ?>')"
                                            ...
                                            title="<?= $checklist['is_active'] ? 'Set Inactive' : 'Set Active' ?>">
                                            <?php if ($checklist['is_active']): ?>
                                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <button type="button" onclick="deleteChecklist(<?= $checklist['id'] ?>, '<?= htmlspecialchars($checklist['name']) ?>')"
    ...
    <div id="checklistModalPanel" class="relative z-10 w-full max-w-xl mx-4 bg-white rounded-2xl shadow-2xl border border-gray-100 transform scale-95 opacity-0 transition-all duration-200 max-h-[90vh] overflow-y-auto">
        <form id="checklistForm" method="POST" action="<?= BASE_URL ?>/master/checklists/store">
            ...
            <div class="flex items-center justify-between border-b border-gray-100 px-6 py-5">
                <div>
                    <h3 id="checklistModalTitle" class="text-lg font-semibold text-gray-900">Create New Checklist Item</h3>
                    <p class="mt-0.5 text-xs text-gray-500">Fill in the checklist item details below.</p>
                </div>
                <button type="button" onclick="closeChecklistModal()" class="rounded-lg p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="p-6 space-y-5">
                <div>
                    <label class="mb-1.5 block text-xs font-semibold text-gray-700">
                        Checklist Item Name <span class="text-red-500">*</span>
                    </label>
                    <input type="text" name="name" id="checklistName" value="<?= old('name') ?>"
                        ...
                        placeholder="e.g. Budget Message" required>
                </div>

                <div>
                    <label class="mb-1.5 block text-xs font-semibold text-gray-700">Description / Notes</label>
                    <textarea name="description" id="checklistDescription" rows="3"
                        ...
                        placeholder="Optional description for this checklist requirement..."><?= old('description') ?></textarea>
                </div>

                <div>
                    <label class="mb-1.5 block text-xs font-semibold text-gray-700">Sort Order</label>
                    <input type="number" name="sort_order" id="checklistSortOrder" min="0" value="<?= old('sort_order', '0') ?>"
                        class="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm text-gray-800 placeholder-gray-400 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
                        placeholder="0">
                    <p class="mt-1 text-[11px] text-gray-500">Lower values appear first in lists.</p>
                </div>

                <div class="flex items-end">
                    <div class="flex items-center gap-3 rounded-xl border border-gray-200 bg-gray-50 p-4 w-full">
                        <input type="checkbox" name="is_active" id="checklistIsActive" value="1" checked
                            class="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary">
                        <div>
                            <label for="checklistIsActive" class="cursor-pointer text-sm font-medium text-gray-800">Set as active</label>
                            <p class="mt-0.5 text-[11px] text-gray-500">Active checklist items appear in selection lists.</p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="flex items-center justify-end gap-3 border-t border-gray-100 px-6 py-4 bg-gray-50 rounded-b-2xl">
                ...
                <button type="submit" id="checklistSubmitBtn"
...
function resetChecklistForm() {
    ...
    document.getElementById('checklistSortOrder').value = '0';
    document.getElementById('checklistIsActive').checked = true;
    form.action = `${BASE}/master/checklists/store`;
    document.getElementById('checklistModalTitle').textContent = 'Create New Checklist Item';
    document.getElementById('checklistSubmitBtn').innerHTML = `
        <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
        </svg>
        Save
...
function openChecklistModal() {
    ...
    setTimeout(() => document.getElementById('checklistName').focus(), 220);
}
...
async function editChecklist(id) {
    try {
        const response = await fetch(`${BASE}/master/checklists/edit?id=${id}`);
        const result = await response.json();
        if (!result.success) {
            showToast(result.message || 'Unable to load checklist item.', 'error');
            return;
        }
        ...
        document.getElementById('checklistDescription').value = item.description || '';
        document.getElementById('checklistSortOrder').value = item.sort_order || '0';
        document.getElementById('checklistIsActive').checked = item.is_active == 1;
        form.action = `${BASE}/master/checklists/update`;
        ...
        showToast('Failed to fetch checklist details.', 'error');
    }
}
...
async function toggleStatus(id, name) {
    openConfirmModal({
        title: 'Toggle Status',
        message: `Are you sure you want to change the status of <strong>"${name}"</strong>?`,
        ...
        onConfirm: async () => {
            try {
                ...
                const response = await fetch(`${BASE}/master/checklists/toggle-status`, {
                    method: 'POST',
                    body: formData
                });
                const result = await response.json();
...
async function deleteChecklist(id, name) {
    openConfirmModal({
        ...
        message: `Are you sure you want to delete <strong>"${name}"</strong>?<br><span class="text-xs text-gray-500">This action cannot be undone.</span>`,
        ...
        onConfirm: async () => {
            try {
                const formData = new FormData();
                formData.append('id', id);
                const response = await fetch(`${BASE}/master/checklists/destroy`, {
                    method: 'POST',
                    body: formData
                });
                const result = await response.json();
                if (result.success) {
                    showToast(result.message || 'Checklist item deleted successfully.', 'success');
                    setTimeout(() => location.reload(), 800);
                } else {
                    showToast(result.message || 'Failed to delete checklist item.', 'error');
                }
            } catch (error) {
