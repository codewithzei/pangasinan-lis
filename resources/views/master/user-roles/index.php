...
$error = $error ?? null;
$pageTitle = $pageTitle ?? 'User Roles Management';
$pageSubtitle = $pageSubtitle ?? '';
$totalRoles = $totalRoles ?? 0;
$activeRoles = $activeRoles ?? 0;
$inactiveRoles = $inactiveRoles ?? 0;
...
<div class="space-y-6">
    ...
    <section class="overflow-hidden rounded-2xl bg-gradient-to-br from-blue-700 via-primary to-indigo-700 shadow-md">
        <div class="relative px-6 py-8 sm:px-8">
            <div class="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                <div class="max-w-3xl">
                    <p class="text-sm font-medium text-blue-100">USER ROLES DATA</p>
    ...
    <section>
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <div class="flex items-start justify-between">
                    <div>
                        <p class="text-sm text-gray-500">Inactive Roles</p>
                        <p class="mt-2 text-3xl font-bold text-gray-900"><?= number_format($inactiveRoles) ?></p>
                    </div>
    ...
    <section class="rounded-2xl border border-gray-200 bg-white">
        <div class="flex flex-col gap-4 border-b border-gray-100 px-6 py-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <h2 class="font-semibold text-gray-900">Roles List</h2>
                <p class="mt-1 text-xs text-gray-500">
                    <?= $totalRows ?> record<?= $totalRows !== 1 ? 's' : '' ?> found
                </p>
            </div>
            <form method="GET" action="<?= BASE_URL ?>/master/user-roles" class="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div class="relative">
                    ...
                    <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Search role name..."
                ...
                <?php if ($search !== '' || $filterStatus !== ''): ?>
                    <a href="<?= BASE_URL ?>/master/user-roles"
                    ...
                    <?php if (empty($roles)): ?>
                        <tr>
                            <td colspan="5" class="px-6 py-16 text-center">
                                ...
                                <p class="mt-4 text-sm font-medium text-gray-700">No roles found.</p>
                                <p class="mt-1 text-xs text-gray-500">Create your first role to populate the list.</p>
                                <button type="button" onclick="openRoleModal()"
                        ...
                        <tr class="hover:bg-gray-50/50 transition">
                            <td class="px-6 py-4">
                                <div>
                                    ...
                                    <?php if (!empty($r['description'])): ?>
                                        <div class="mt-0.5 text-xs text-gray-500 line-clamp-1 max-w-lg"><?= htmlspecialchars($r['description']) ?></div>
                                    <?php endif; ?>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    ...
                                    <button type="button" onclick="toggleStatus(<?= $r['id'] ?>, '<?= htmlspecialchars($r['name']) ?>')"
                                        ...
                                        title="<?= $r['is_active'] ? 'Set Inactive' : 'Set Active' ?>">
                                        <?php if ($r['is_active']): ?>
                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                  d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"/>
...
<div id="userRoleModal" class="fixed inset-0 z-[100] hidden items-center justify-center" role="dialog" aria-modal="true">
    <div id="userRoleModalBackdrop" class="absolute inset-0 bg-gray-900/50 backdrop-blur-sm transition-opacity opacity-0"></div>
    <div id="userRoleModalPanel" class="relative z-10 w-full max-w-xl mx-4 bg-white rounded-2xl shadow-2xl border border-gray-100 transform scale-95 opacity-0 transition-all duration-200 max-h-[90vh] overflow-y-auto">
        <form id="userRoleForm" method="POST" action="<?= BASE_URL ?>/master/user-roles/store">
            <input type="hidden" name="id" id="userRoleId" value="">
            <div class="flex items-center justify-between border-b border-gray-100 px-6 py-5">
                <div>
                    <h3 id="userRoleModalTitle" class="text-lg font-semibold text-gray-900">Create New User Role</h3>
                    <p class="mt-0.5 text-xs text-gray-500">Fill in the user role details below.</p>
                </div>
                <button type="button" onclick="closeUserRoleModal()" class="rounded-lg p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="p-6 space-y-5">
                    <div class="sm:col-span-full">
                        ...
                        <input type="text" name="name" id="roleName" value="<?= old('name') ?>"
                ...
                <div>
                    ...
                    <textarea name="description" id="roleDescription" rows="3"
                        ...
                        placeholder="Optional description or coverage notes about this role..."><?= old('description') ?></textarea>
                </div>
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="number" name="sort_order" id="sortOrder" min="0" value="<?= old('sort_order', '0') ?>"
                            class="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm text-gray-800 placeholder-gray-400 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
                            placeholder="0">
                        <p class="mt-1 text-[11px] text-gray-500">Roles with lower values appear first in dropdowns.</p>
                    </div>
                    <div class="flex items-end">
                        <div class="flex items-center gap-3 rounded-xl border border-gray-200 bg-gray-50 p-4 w-full">
                            ...
                            <div>
                                ...
                                <p class="text-[11px] text-gray-500 mt-0.5">Active roles appear in selection dropdowns.</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex items-center justify-end gap-3 border-t border-gray-100 px-6 py-4 bg-gray-50 rounded-b-2xl">
                <button type="button" onclick="closeUserRoleModal()"
                ...
                <button type="submit" id="userRoleSubmitBtn"
...
function attachRoleValidation() {
    const fields = ['roleName', 'roleNumber', 'sortOrder'];
    fields.forEach(id => {
        const el = document.getElementById(id);
        if (!el) return;
        el.addEventListener('input', validateRoleForm);
        el.addEventListener('blur', validateRoleForm);
        el.addEventListener('change', validateRoleForm);
    });
    document.getElementById('userRoleForm')?.addEventListener('submit', (e) => {
        if (!validateRoleForm()) {
            e.preventDefault();
...
function resetRoleForm() {
    ...
    form.action = '<?= BASE_URL ?>/master/user-roles/store';
    document.getElementById('userRoleModalTitle').textContent = 'Create New Role';
    document.getElementById('userRoleSubmitBtn').innerHTML = `
        <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
        </svg>
        Save
...
function openRoleModal() {
    if (userRoleModalOpen) return;
    resetRoleForm();
    userRoleModalOpen = true;
    const m = document.getElementById('userRoleModal');
    const b = document.getElementById('userRoleModalBackdrop');
    const p = document.getElementById('userRoleModalPanel');
    ...
    setTimeout(() => document.getElementById('roleName').focus(), 220);
}
...
document.getElementById('userRoleModalBackdrop')?.addEventListener('click', closeUserRoleModal);
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
        if (userRoleModalOpen) closeUserRoleModal();
...
async function editRole(id) {
    try {
        const res = await fetch(`<?= BASE_URL ?>/master/user-roles/edit?id=${id}`);
        const data = await res.json();
        if (!data.success) {
            showToast(data.message || 'Role not found.', 'error');
            return;
        }
        const r = data.data;
        document.getElementById('userRoleId').value = r.id;
        document.getElementById('roleName').value = r.name;
        document.getElementById('roleDescription').value = r.description || '';
        document.getElementById('sortOrder').value = r.sort_order ?? 0;
        document.getElementById('isActive').checked = !!r.is_active;
        document.getElementById('userRoleForm').action = '<?= BASE_URL ?>/master/user-roles/update';
        document.getElementById('userRoleModalTitle').textContent = 'Edit Role';
        document.getElementById('userRoleSubmitBtn').innerHTML = `
            <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        const m = document.getElementById('userRoleModal');
        const b = document.getElementById('userRoleModalBackdrop');
        const pn = document.getElementById('userRoleModalPanel');
        m.classList.remove('hidden');
        m.classList.add('flex');
        requestAnimationFrame(() => {
            b.classList.remove('opacity-0');
...
function openConfirmModal({ title, message, confirmText = 'Confirm', cancelText = 'Cancel', type = 'danger', onConfirm }) {
    ...
    if (type === 'danger') {
        ...
        proceedBtn.className = 'inline-flex items-center justify-center rounded-xl px-4 py-2 text-sm font-semibold text-white bg-emerald-600 hover:bg-emerald-700 transition shadow-sm';
    ...
    openConfirmModal({
        ...
        onConfirm: async () => {
            try {
                ...
                fd.append('id', id);
                const res = await fetch('<?= BASE_URL ?>/master/user-roles/destroy', { method: 'POST', body: fd });
                const data = await res.json();
...
async function toggleStatus(id, name) {
    openConfirmModal({
        title: 'Toggle Status',
        message: `This will toggle the active status of <strong class="text-gray-800">${name}</strong>.<br><span class="text-xs text-gray-500">Inactive roles are hidden from selection dropdowns.</span>`,
        confirmText: 'Proceed',
        type: 'info',
        onConfirm: async () => {
            try {
                const fd = new FormData();
                fd.append('id', id);
                const res = await fetch('<?= BASE_URL ?>/master/user-roles/toggle-status', { method: 'POST', body: fd });
                const data = await res.json();
                showToast(data.message, data.success ? 'success' : 'error');
